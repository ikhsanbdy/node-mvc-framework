...
/**
 * Form Validator
 * Source: https://github.com/iwakme/iwak-framework/blob/master/lib/validator.js at 2017-07-12
...
const getResult = function(data, errors) {
	return {
		...
		get: function(key, defVal) {
			if (!key) return data;
			else return (typeof data[key] !== 'undefined' ? data[key] : defVal);
		}
	};
};
...
const validateSchema = function(objData, param, schema) {
	try {
		...
		if (schema.isRequired) {
			if (typeof data === 'undefined' || data.length === 0) return getError(param, {
				type: 'required'
...
Validator.unescape = function(str) {
	return validator.unescape(str);
}

Validator.toBoolean = function(str) {
	return validator.toBoolean(str);
}
